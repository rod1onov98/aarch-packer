...
inline void write_a64_stub_pcrel(uint8_t* p, uint64_t stub_va, uint64_t target) {
    int64_t page_off = (int64_t)(target & ~0xfff) - (int64_t)(stub_va & ~0xfff);
    int64_t imm = page_off >> 12;                 // 21-bit signed (±4GB)
    uint32_t immlo = (uint32_t)(imm & 0x3);
    uint32_t immhi = (uint32_t)((imm >> 2) & 0x7FFFF);
    uint32_t adrp = 0x90000000 | (immlo << 29) | (immhi << 5) | 16; // x16
    uint32_t lo12 = (uint32_t)(target & 0xfff);
    uint32_t add  = 0x91000000 | (lo12 << 10) | (16 << 5) | 16;    // add x16,x16,#lo12
