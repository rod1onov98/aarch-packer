...
static const uint32_t PROT_MAGIC_RAW = 0x544F5250;
static const uint32_t PROT_MAGIC_MASK = 0xC01EBA42;
static const uint32_t WLIST_MAGIC = 0x54534C57;
                    ...
                    if(rels.empty()){
                        ...
                        for(auto&r:rels){
                            ...
                            if(rtype==RT_ABS64 && rsym){
                                // resolve symbol value from .dynsym (sh_link of this RELA section)
                                uint64_t symval=0;
                        ...
                        for(size_t i=0;i<orig.size();i++){
                            if((uint64_t)orig[i]==prot_init_va) continue; // skip prot_init own slot
